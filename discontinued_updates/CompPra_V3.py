# ATTEMPT TO UPGRADE THE PLOTTING FUNCITONALITY TO ALLOW FOR PLAYBACK
# OF THE SIMULATION. I HAVE FAILED TO DO SO AND MOVED ONTO DIFFERENT 
# FUNCITONALITIES THAT ARE MORE MEANGINGFUL FOR THE MOMENT
...
def init_grid(grid_size, max_attractant_val):
    u = np.zeros((nodes, nodes))
    max_temp = max_attractant_val
...
cell_history = [cell.get_position_history("list") for cell in cells]
...
def update(frame):
    hist_diff = u_history[:frame]
    cell_pos = cell_history[:,1:]
    #update
    pcm.set_array(hist_diff[frame].ravel())
    axis.set_title("Distribution at t: {:.3f} [s].".format(frame))
    axis.plot(cell_pos[frame][:,0], cell_pos[frame][:,1], 'wo', markersize=1)

    return (pcm, axis)

ani = FuncAnimation(fig = fig, func=update, frames = sim_time, interval = dt)
plt.show()
